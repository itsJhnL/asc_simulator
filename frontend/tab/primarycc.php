...
<<<<<<< HEAD
    ...
    function selectClarificationCode() {
        console.log("OK button clicked!"); // Debugging

        const selectElement = document.getElementById('valueSelect');
        const selectedValue = selectElement.value;

        if (!selectedValue) {
            alert("Please select a clarification code before proceeding.");
            return;
        }

        console.log("Selected Clarification Code:", selectedValue);

        // Store the selected clarification code in localStorage
        localStorage.setItem("clarification_code", selectedValue);

        // Close the modal after selection
        const modalElement = document.getElementById('ccModal');
        const modal = bootstrap.Modal.getInstance(modalElement) || new bootstrap.Modal(modalElement);
        modal.hide();
    }
    document.addEventListener('DOMContentLoaded', (event) => {
        updateDescription(); // Set initial description based on the default selected option
    });
</script>

    <!--end of clarification code modal-->
...
<script>
    $(document).ready(function () {
        $("#submitForm").submit(function (event) {
            ...
            let clarificationCode = localStorage.getItem("clarification_code"); // Retrieve from localStorage
            ...
            $.ajax({
                type: "POST",
                url: "functioncc.php", // Ensure this points to the correct PHP file
                data: {
                    qty: quantity,
